...
import { renderAthleteHome, renderMetrics, renderCommunity, renderProfile } from './athlete.js';
...
export function setupUI() {
    ...
    document.getElementById('btn-login-athlete')?.addEventListener('click', () => import('./auth.js').then(m => m.login('athlete')));
    document.getElementById('btn-login-coach')?.addEventListener('click', () => import('./auth.js').then(m => m.login('coach')));
    document.getElementById('btn-login-admin')?.addEventListener('click', () => import('./auth.js').then(m => m.login('admin')));
...
export function renderHeader() {
    ...
    document.getElementById('header-user-role').innerText = user.roleDesc || user.role;
...
export function renderBottomNav(role) {
    ...
    if (role === 'athlete') {
        ...
        nav.className = "fixed bottom-0 w-full bg-slate-900 border-t border-slate-800 flex justify-around py-4 z-50";
        nav.innerHTML = `
